...
export function setupIbcV2Extension(base: QueryClient): IbcV2Extension {
  ...
  return {
    ibc: {
      clientV2: {
        counterparty: async (clientId: string) => clientQueryService.CounterpartyInfo({
          clientId,
        }),
      },
      channelV2: {
        ...
        unreceivedPackets: async (
          ...
          clientId,
          sequences: sequences.map(s => BigInt(s)),
        }),
        ...
        nextSequenceSend: async (clientId: string) => channelQueryService.NextSequenceSend({
          clientId,
        }),
      },
    },
