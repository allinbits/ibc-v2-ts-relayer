...
export interface IbcExtension {
  readonly ibc: {
    ...
    readonly client: {
      ...
      readonly consensusState: (clientId: string, height?: Height) => Promise<QueryConsensusStateResponse>
...
export function setupIbcExtension(base: QueryClient): IbcExtension {
  ...
  return {
    ibc: {
      ...
      client: {
        state: async (clientId: string) => clientQueryService.ClientState({
          clientId,
        }),
        ...
        consensusState: async (clientId: string, consensusHeight?: Height) =>
