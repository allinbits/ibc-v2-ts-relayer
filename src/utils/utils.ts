...
export function toIntHeight(height?: Height): number {
  return height?.revisionHeight ? Number(height.revisionHeight) : 0;
}
...
export function mapRpcPubKeyToProto(pubkey?: RpcPubKey): ProtoPubKey | undefined {
  if (pubkey === undefined) {
    return undefined;
  }
...
export function secondsFromDateNanos(date: ReadonlyDateWithNanoseconds): number {
...
export async function prepareConnectionHandshake(
  ...
  const proof = await src.getConnectionHandshakeProof(clientIdSrc, connIdSrc, headerHeight);
  return proof;
}
...
export function decodeClientState(clientState: Any) {
...
export function decodeConsensusState(consensusState: Any | undefined) {
