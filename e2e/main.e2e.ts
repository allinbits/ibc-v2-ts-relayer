...
const init = async () => {
  ...
  await relayer.addNewdRelayPath("mars", "http://[::1]:26657", "venus", "http://[::1]:26658", ChainType.Cosmos, ChainType.Cosmos, 2);
};
...
test("Start relayer and. run E2E tests", async () => {
  ...
  const tmClientA = await connectComet("http://[::1]:26657");
  const tmClientB = await connectComet("http://[::1]:26658");
